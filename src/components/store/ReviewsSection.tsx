...
export function ReviewsSection() {
  const avg = DEMO_REVIEWS.reduce((sum, r) => sum + r.rating, 0) / DEMO_REVIEWS.length;

  return (
    <section className="px-4 py-5 border-t border-neutral-100">
      <div className="flex items-center gap-2 mb-3">
        <h2 className="font-bold text-neutral-900">Reviews</h2>
        <div className="flex items-center gap-1 text-sm text-neutral-600">
          <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
          <span>{avg.toFixed(1)}</span>
          <span className="text-neutral-400">· {DEMO_REVIEWS.length} reviews</span>
        </div>
      </div>
